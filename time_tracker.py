...
class TimeTrackerApp:
    ...
    def write_markdown_log(self, project_link_md, start, end, duration):
        ...
        filename = f"{today_formatted}-Work-Log.md"
        ...
        try:
            with open(filepath, "a", encoding="utf-8") as f:
                if not file_exists:
                    # Write initial header using the new format
                    f.write(f"# ⏳ Work Log for {today_formatted}\n\n")
                    f.write("| Project | Start | End | Duration |\n")
                    f.write("| :--- | :--- | :--- | :--- |\n")
                
                # Append the new session row, using the wikilink variable
                f.write(f"| {project_link_md} | {start} | {end} | {duration} |\n")
